...
const SingleDeck = () => {
  ...
  const fetchCards = async (query: URLSearchParams) => {
    ...
    try {
      ...
      const sortedData = data
        ...
        .sort(function (a: PokemonTCG.Card, b: PokemonTCG.Card) {
          return (
            Number(new Date(b.set.releaseDate)) -
            Number(new Date(a.set.releaseDate))
          );
        });
  ...
  const { sortedPokemon, sortedTrainers, sortedEnergy } = deckCards.length
    ? deckSorting(deckCards)
    : deckSorting([]);
  ...
  return (
    <main className="page_container">
      ...
      {showOverlayCard ? (
        ...
        >
          <div className="flex flex-col items-center justify-center w-full px-8">
            <img src={overlayCard} className="max-w-full sm:max-w-md" />
      ...
      {showForm ? (
        <form
          className="text-black flex flex-col items-start w-full p-2 bg-slate-400 bg-opacity-20"
        ...
        >
          <div className="flex flex-wrap gap-2 w-full lg:justify-center sm:flex-nowrap">
            ...
            <div className="search_option">
              <label htmlFor="supertype" className="text-xl font-semibold">
                Card Type:{" "}
              </label>
              <select
                id="supertype"
              ...
              >
                ...
                {superTypes.map((supertype) => (
                  <option value={supertype} key={supertype}>
                    {supertype}
                  </option>
                ))}
              </select>
          ...
          <div className="flex flex-wrap gap-2 mt-4 w-full lg:justify-center sm:flex-nowrap">
            <div className="search_option">
              <label htmlFor="setname" className="text-xl font-semibold">
                Set Name:{" "}
              </label>
              <select
                id="setname"
              ...
              >
                ...
                {sortedCardSets.map((sets) => (
                  <option value={sets.name} key={sets.name + sets.series}>
            ...
            <div className="search_option">
              <label htmlFor="format" className="text-xl font-semibold">
                Format:{" "}
              </label>
              <select
                id="format"
      ...
      {noResults ? (
        <div className="text-2xl text-red-500">No results</div>
      ) : null}
      ...
      {cardData.length && showResults ? (
        <section className="flex flex-col justify-center items-center gap-2 bg-slate-400 bg-opacity-20 py-4 rounded-b-lg">
      ...
      <div className="flex gap-1 self-start justify-self-start items-center mt-4 ml-8 text-black">
        <Switch
          ...
          checked={viewMode}
          checkedIcon={
            <TbCardsFilled className="w-full h-full p-1 text-white" />
          }
          uncheckedIcon={
            <BsFillGridFill className="w-full h-full p-1 text-white" />
          }
        />
        <h4>{viewMode ? "Stacked" : "Spread"}</h4>
      </div>

      <h3 className="my-2 text-slate-800 font-bold text-2xl">
        Pokemon {sortedPokemon.length}
      </h3>
